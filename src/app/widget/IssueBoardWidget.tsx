...
interface WidgetIssue {
  ...
  sender: string;      // latest editor (last changed by)
    ...
    <div style={s.overlay} role="dialog" aria-modal="true" aria-label={isNew ? 'New issue' : 'Edit issue'} onKeyDown={handleKeyDown}>
      <div style={s.dialog}>
        ...
        <div style={s.dialogBody}>
          {/* Creator / last changed metadata */}
          {!isNew && creatorId && (
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 8, fontSize: 12, color: '#777' }}>
              <span>Created by <strong>{shortUserId(creatorId)}</strong></span>
              {lastChangedById && lastChangedById !== creatorId && (
                <span>Last changed by <strong>{shortUserId(lastChangedById)}</strong></span>
              )}
            </div>
          )}
          {/* Form fields */}
...
interface KanbanViewProps {
  ...
  onHideColumn: (label: string) => void;
}

function KanbanView({ issues, schema, kanbanField, reactionsByIssue, commentCountByIssue, onEdit, hiddenColumns, onHideColumn }: KanbanViewProps) {
  const titleField = schema.fields.find(f => f.key === 'title') ?? schema.fields[0];
  ...
  const visibleColumns = columns.filter(col => !hiddenColumns.has(col.label));
          ...
          <div style={s.kanbanColHeader}>
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <button
                ...
                onClick={() => onHideColumn(col.label)}
                style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '0 2px',
                  color: 'inherit', opacity: 0.5, fontSize: '0.9em', lineHeight: 1 }}
              >✕</button>
            </span>
          </div>
...
export function IssueBoardWidget({ widgetApi }: Props) {
  ...
  const hideColumn = (label: string) => setHiddenColumns(prev => new Set([...prev, label]));
  const showColumn = (label: string) => setHiddenColumns(prev => { const next = new Set(prev); next.delete(label); return next; });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [schemaEvents, issueEvents, plEvents, commentEvents, reactionEvents, issueHistoryEvents] = await Promise.all([
          ...
          (widgetApi as any).readRoomEvents('eu.kiefte.issue', 10000).catch(() => [] as IRoomEvent[]),
        ]);
        if (cancelled) return;

        // Build creatorId map: first sender per state key from timeline history
        const creatorMap = new Map<string, string>();
        for (const e of (issueHistoryEvents as IRoomEvent[]).sort((a, b) => a.origin_server_ts - b.origin_server_ts)) {
          if (e.state_key && !creatorMap.has(e.state_key)) creatorMap.set(e.state_key, e.sender);
        }
        ...
        setIssues(
          issueEvents
            ...
            .map(e => ({
              ...
              sender: e.sender,
              creatorId: creatorMap.get(e.state_key!) ?? e.sender,
    ...
    const handler = (actionEv: any) => {
      ...
      if (event.type === 'eu.kiefte.issues.schema') {
        ...
        if (c?._deleted) {
          ...
          setIssues(prev => {
            const existing = prev.find(i => i.stateKey === event.state_key!);
            ...
            return [
              ...rest,
              {
                ...
                content: c,
                sender: event.sender,
                creatorId: existing?.creatorId ?? event.sender,
    ...
    <div style={s.container}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginLeft: 'auto', flexWrap: 'wrap' }}>
          {kanbanField && (
            <>
              ...
              {view === 'kanban' && hiddenColumns.size > 0 && [...hiddenColumns].map(label => (
                <button key={label} onClick={() => showColumn(label)} style={s.btn} aria-label={'Show ' + label + ' column'}>
                  {label}
                </button>
              ))}
            </>
      ...
      {editing !== null && (
        <IssueForm
          ...
          lastChangedById={editing !== 'new' && editing.sender !== editing.creatorId ? editing.sender : undefined}
