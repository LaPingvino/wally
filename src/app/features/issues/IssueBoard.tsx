...
type IssueEntry = { event: MatrixEvent; content: IssueContent; redacted: boolean; creatorId: string; lastChangedById: string };
...
function getIssues(room: Room): IssueEntry[] {
  ...
  // Build a map of stateKey → earliest sender (creator) from timeline
  const creatorMap = new Map<string, string>();
  for (const e of timelineEvents) {
    if (e.getType() === 'eu.kiefte.issue') {
      const sk = e.getStateKey()!;
      if (!creatorMap.has(sk)) creatorMap.set(sk, e.getSender() ?? '');
    }
  }
  ...
  for (const event of stateEvents) {
    ...
    // Fall back to current event's sender if not found in timeline (e.g. before pagination)
    const creatorId = creatorMap.get(stateKey) ?? event.getSender() ?? '';
    const lastChangedById = event.getSender() ?? creatorId;
    if (!event.isRedacted()) {
      const content = event.getContent() as IssueContent;
      if (!content._deleted) result.push({ event, content, redacted: false, creatorId, lastChangedById });
    } else {
      ...
      for (let i = timelineEvents.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          if (!c._deleted) {
            result.push({ event, content: c, redacted: true, creatorId, lastChangedById: e.getSender() ?? creatorId });
            break;
          }
        }
      }
...
function loadViewMode(roomId: string, schema: IssueSchema | null): ViewMode {
  try {
    const stored = localStorage.getItem('wally-issue-view-' + roomId);
...
function FieldValue({ field, value, room }: { field: SchemaField; value: unknown; room: Room }) {
  ...
  return <Text size="T200" style={{ wordBreak: 'break-word' }}>{str}</Text>;
}
...
function IssueForm({ schema, initial, room, issueId, creatorId, lastChangedById, onSave, onCancel, canDelete, onDelete }: IssueFormProps) {
  ...
  return (
    <Dialog role="dialog" aria-modal="true" aria-labelledby={titleId} variant="Surface" style={{ width: '90vw', maxWidth: '520px' }}>
      ...
      <Scroll style={{ maxHeight: '70vh' }}>
        {creatorId && (
          <Box gap="300" alignItems="Center" style={{ flexWrap: 'wrap', padding: config.space.S200 + ' ' + config.space.S400 + ' 0' }}>
            <Box gap="100" alignItems="Center">
              <Text size="T200" priority="300">Created by</Text>
              <Text size="T200"><MemberDisplay room={room} userId={creatorId} /></Text>
            </Box>
            {lastChangedById && lastChangedById !== creatorId && (
              <Box gap="100" alignItems="Center">
                <Text size="T200" priority="300">Last changed by</Text>
                <Text size="T200"><MemberDisplay room={room} userId={lastChangedById} /></Text>
              </Box>
            )}
          </Box>
        )}
...
function KanbanCard({ entry, schema, room, kanbanFieldKey, canWrite, onClick, isSelected, onToggleSelect, showCheckbox, allColumns, onMoveToColumn }: {
  entry: IssueEntry; schema: IssueSchema; room: Room; kanbanFieldKey: string; canWrite: boolean; onClick: () => void;
  isSelected?: boolean; onToggleSelect?: () => void; showCheckbox?: boolean;
  allColumns: Array<{ key: string; label: string }>; onMoveToColumn: (targetColumn: string) => void;
}) {
  const issueId = entry.event.getStateKey()!;
  const colVal = (entry.content[kanbanFieldKey] as string | undefined) ?? '__uncategorized__';
  const { ref, isDragging, closestEdge } = useKanbanCardDnD(issueId, colVal, kanbanFieldKey, canWrite);
  const titleField = schema.fields.find((f) => f.key === 'title') ?? schema.fields[0];
  const title = titleField ? ((entry.content[titleField.key] as string) ?? 'issue') : 'issue';

  const dropLine = (
    <div style={{ position: 'absolute', left: 0, right: 0, height: 2,
      background: color.Primary.Main, borderRadius: 2, pointerEvents: 'none' }} />
  );

  return (
    <div ref={ref} style={{ opacity: isDragging ? 0.4 : 1, cursor: canWrite ? 'grab' : undefined,
      position: 'relative' }}>
      {closestEdge === 'top' && <div style={{ position: 'absolute', top: -1, left: 0, right: 0 }}>{dropLine}</div>}
      <IssueCard entry={entry} schema={schema} room={room} kanbanFieldKey={kanbanFieldKey}
        canWrite={canWrite} onClick={onClick} showCheckbox={showCheckbox}
        isSelected={isSelected} onToggleSelect={onToggleSelect} />
      {closestEdge === 'bottom' && <div style={{ position: 'absolute', bottom: -1, left: 0, right: 0 }}>{dropLine}</div>}
      {canWrite && allColumns.length > 1 && (
        <select
          value={colVal}
          aria-label={'Move "' + title + '" to column'}
          onChange={(e) => { if (e.target.value !== colVal) onMoveToColumn(e.target.value); }}
          style={{ position: 'absolute', width: '1px', height: '1px', padding: 0, margin: '-1px',
            overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap', border: 0 }}>
          {allColumns.map((col) => (
            <option key={col.key} value={col.key}>{col.label.replace(/_/g, ' ')}</option>
          ))}
        </select>
      )}
    </div>
...
function KanbanColumnSection({ columnValue, kanbanFieldKey, colHeadingId, label, entryCount, onHide, children }: {
  columnValue: string; kanbanFieldKey: string; colHeadingId: string; label: string;
  entryCount: number; onHide: () => void; children: React.ReactNode;
}) {
  const { ref, isOver } = useKanbanDropTarget(columnValue, kanbanFieldKey);

  return (
    <section ref={ref as React.RefObject<HTMLElement>} aria-labelledby={colHeadingId}
      style={{ minWidth: '240px', maxWidth: '320px', flexShrink: 0,
        background: isOver ? color.Primary.Container : color.SurfaceVariant.Container,
        outline: isOver ? '2px solid ' + color.Primary.Main : undefined,
        borderRadius: config.radii.R300, padding: config.space.S300,
        display: 'flex', flexDirection: 'column', gap: config.space.S200,
        transition: 'background 0.15s' }}>
      <Box alignItems="Center" style={{ justifyContent: 'space-between' }}>
        <Text id={colHeadingId} as="h3" size="H6">{label.replace(/_/g, ' ')}</Text>
        <Box alignItems="Center" gap="100">
          <Text size="T200" priority="300" aria-label={entryCount + ' issues'}>{entryCount}</Text>
          <button type="button" aria-label={'Hide ' + label + ' column'} onClick={onHide}
            style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '2px 4px',
              color: 'inherit', opacity: 0.5, fontSize: '0.9em', lineHeight: 1 }}>✕</button>
        </Box>
      </Box>
      {children}
    </section>
...
function IssueListItem({ entry, schema, room, canWrite, onEdit, isSelected, onToggleSelect, showCheckbox }: {
  ...
  const { content, redacted, creatorId, lastChangedById } = entry;
  const titleField = schema.fields.find((f) => f.key === 'title') ?? schema.fields[0];
  const title = titleField ? ((content[titleField.key] as string) ?? '(untitled)') : '(untitled)';
    ...
    <Box direction="Column" gap="200"
      ...
      <Box alignItems="Center" gap="200" style={{ justifyContent: 'space-between', padding: config.space.S300 + ' ' + config.space.S300 + ' 0' }}>
        <Box alignItems="Center" gap="100">
          ...
          <Text size="T300" as="h3" style={{ margin: 0, fontWeight: 'bold', wordBreak: 'break-word' }}>{title}</Text>
      ...
      <Box gap="200" style={{ flexWrap: 'wrap', padding: '0 ' + config.space.S300 }}>
        <Box gap="100" alignItems="Center">
          <Text size="T200" priority="300">Created by</Text>
          <Text size="T200"><MemberDisplay room={room} userId={creatorId} /></Text>
        </Box>
        {lastChangedById !== creatorId && (
          <Box gap="100" alignItems="Center">
            <Text size="T200" priority="300">Last changed by</Text>
            <Text size="T200"><MemberDisplay room={room} userId={lastChangedById} /></Text>
          </Box>
...
export function IssueBoard({ room }: IssueBoardProps) {
  ...
  const getKanbanOrder = useCallback((kanbanFieldKey: string): Record<string, string[]> => {
    const ev = room.getLiveTimeline().getState(EventTimeline.FORWARDS)
      ?.getStateEvents('eu.kiefte.issues.order' as any, kanbanFieldKey);
    return (ev?.getContent() as Record<string, string[]> | undefined) ?? {};
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [room, tick]);

  const handleKanbanDrop = useCallback(async (
    { issueId, sourceColumn, targetColumn, kanbanFieldKey, targetCardId, targetEdge }: KanbanDropResult
  ) => {
    if (!canWriteIssues) return;
    const entry = issues.find((e) => e.event.getStateKey() === issueId);
    if (!entry) return;

    const order = getKanbanOrder(kanbanFieldKey);
    const getColIds = (col: string): string[] => {
      if (order[col]?.length) return [...order[col]];
      return issues
        .filter((e) => col === '__uncategorized__' ? !e.content[kanbanFieldKey] : e.content[kanbanFieldKey] === col)
        .map((e) => e.event.getStateKey()!);
    };

    const srcIds = getColIds(sourceColumn).filter((id) => id !== issueId);
    const dstIds = sourceColumn === targetColumn ? [...srcIds] : getColIds(targetColumn).filter((id) => id !== issueId);

    if (targetCardId) {
      const idx = dstIds.indexOf(targetCardId);
      dstIds.splice(idx >= 0 ? (targetEdge === 'bottom' ? idx + 1 : idx) : dstIds.length, 0, issueId);
    } else {
      dstIds.push(issueId);
    }

    const newOrder = { ...order, [sourceColumn]: srcIds, [targetColumn]: dstIds };

    if (sourceColumn !== targetColumn) {
      const newVal = targetColumn === '__uncategorized__' ? undefined : targetColumn;
      await mx.sendStateEvent(room.roomId, 'eu.kiefte.issue' as any,
        { ...entry.content, [kanbanFieldKey]: newVal }, issueId);
    }
    await mx.sendStateEvent(room.roomId, 'eu.kiefte.issues.order' as any, newOrder, kanbanFieldKey);
  }, [canWriteIssues, issues, mx, room.roomId, getKanbanOrder]);

  useKanbanMonitor(handleKanbanDrop, schema !== null && viewMode.type === 'kanban');
  ...
  const renderKanban = (kanbanFieldKey: string) => {
    ...
    const order = getKanbanOrder(kanbanFieldKey);
    const applyOrder = (entries: IssueEntry[], colKey: string): IssueEntry[] => {
      const ids = order[colKey];
      if (!ids?.length) return entries;
      const pos = new Map(ids.map((id, i) => [id, i]));
      return [...entries].sort((a, b) =>
        (pos.get(a.event.getStateKey()!) ?? Infinity) - (pos.get(b.event.getStateKey()!) ?? Infinity)
      );
    };
    ...
    const columns: Column[] = [
      ...(kanbanField.values ?? []).map((value) => ({
        ...
        entries: applyOrder(issues.filter((entry) => entry.content[kanbanField.key] === value), value),
      })),
      { key: '__uncategorized__', label: 'Uncategorized',
        entries: applyOrder(issues.filter((entry) => !kanbanField.values?.includes(entry.content[kanbanField.key] as string)), '__uncategorized__') },
    ].filter((col) => col.entries.length > 0 || kanbanField.values?.includes(col.key));

    const visibleColumns = columns.filter((col) => !hiddenColumns.has(col.key));
    const allColumnOptions = columns.map((c) => ({ key: c.key, label: c.label }));
      ...
      <div style={{ flexGrow: 1, overflow: 'auto' }} role="region" aria-label="Issue board">
        <Box gap="300" style={{ padding: config.space.S300, minHeight: '100%', alignItems: 'flex-start' }}>
          {visibleColumns.map((col) => {
            ...
            return (
              <KanbanColumnSection key={col.key} columnValue={col.key} kanbanFieldKey={kanbanFieldKey}
                colHeadingId={colHeadingId} label={col.label} entryCount={col.entries.length}
                onHide={() => setHiddenColumns((prev) => new Set([...prev, col.key]))}>
                    ...
                    return (
                      <li key={sk}>
                        <KanbanCard
                          ...
                          onMoveToColumn={(targetColumn) => handleKanbanDrop({
                            issueId: sk, sourceColumn: col.key, targetColumn, kanbanFieldKey,
                            targetCardId: null, targetEdge: null,
                          })}
                        />
                      </li>
                    );
  ...
  return (
    <Box grow="Yes" direction="Column" style={{ overflow: 'hidden' }}>
      ...
      {/* Secondary toolbar: kanban group-by selector + hidden column restore buttons */}
      {effectiveViewMode.type === 'kanban' && (enumFields.length > 1 || hiddenColumns.size > 0) && (
        <Box shrink="No" alignItems="Center" gap="200" style={{ flexWrap: 'wrap',
          ...
          {hiddenColumns.size > 0 && (() => {
            const kanbanField = effectiveViewMode.type === 'kanban'
              ? schema.fields.find((f) => f.key === effectiveViewMode.fieldKey)
              : undefined;
            const allCols = [
              ...(kanbanField?.values ?? []).map((v) => ({ key: v, label: v })),
              { key: '__uncategorized__', label: 'Uncategorized' },
            ];
            return allCols
              .filter((col) => hiddenColumns.has(col.key))
              .map((col) => (
                <Button key={col.key} size="300" variant="Secondary" fill="Soft"
                  onClick={() => setHiddenColumns((prev) => { const next = new Set(prev); next.delete(col.key); return next; })}
                  aria-label={'Show ' + col.label + ' column'}>
                  <Text>{col.label.replace(/_/g, ' ')}</Text>
                </Button>
              ));
          })()}
        </Box>
