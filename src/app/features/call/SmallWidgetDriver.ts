...
export class SmallWidgetDriver extends WidgetDriver {
  ...
  // Cache the result so we only call getVersions() once per driver instance
  ...
  private async checkDelayedEventSupport(): Promise<boolean> {
    ...
    try {
      ...
      // MSC4157 revised the original MSC4140 proposal; check both flag names
  ...
  public async validateCapabilities(requested: Set<Capability>): Promise<Set<Capability>> {
    ...
    // Only grant delayed-event capabilities when the server actually supports them.
    // If the server doesn't support MSC4157/MSC4140, EC falls back to regular state events
    // for membership at capability-grant time (the correct fallback level). Granting these
    // on a server with broken support causes EC's MembershipManager to retry updateDelayedEvent
    // 10 times then throw "Connection lost", crashing the call.
  ...
  /**
   * @experimental Part of MSC4140 & MSC4157
   * matrix-widget-api 1.13+ calls this single method with an action argument.
   */
  ...
  // Kept for backwards compat with older widget-api versions
  ...
  /**
   * Yields TURN servers from the homeserver, then streams updates.
   * Returns immediately if no TURN servers are available, letting EC
   * handle the no-TURN case gracefully (avoids hanging widget init).
   */
  public async *getTurnServers(): AsyncGenerator<ITurnServer> {
    const client = this.mxClient;
    const toWidget = (s: MxTurnServer): ITurnServer => ({
      uris: s.urls,
      username: s.username,
    ...
    for (const server of initial) {
      yield toWidget(server);
    }
    // If no TURN servers, return so EC can proceed without them
    if (initial.length === 0) return;

    // There were servers — stream refreshes as the client polls
