// Module augmentation: register Wally/cinny custom account-data event types into
// the SDK's `AccountDataEvents` map. Without this, `mx.getAccountData(...)` /
// `mx.setAccountData(...)` for these app-specific types fail typechecking
// ("Argument of type '…' is not assignable to parameter of type
// 'keyof AccountDataEvents'") and call sites resort to `as any`. The content
// types here are the same ones the call sites already read/write, so reads via
// `getContent<T>()` and writes both line up.
//
